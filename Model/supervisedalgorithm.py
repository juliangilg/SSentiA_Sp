...
class TF_IDF(object):
    ...
    def get_tf_idf(self, X):   
        vectorizer = TfidfVectorizer(ngram_range=(1,2), sublinear_tf=True, min_df=5, norm='l2', tokenizer=lambda x: x.split())
...
class MultinomialNB_Classifier(object):
    def predict(self, X_train, Y_train,  X_test):
        classifier =  ComplementNB()  #Pipeline([("vect", CountVectorizer(stop_words="english")),("tfidf", TfidfTransformer()),("clf", MultinomialNB())])
